...
public class MensajeEmpleadoApi {
	...
	//curl -v http://localhost:8080/03a_JakartaEE-Authorization/mensajes/Empleado/enviarMensaje?mensaje=HolaMundo
	//con la url anterior devuelve 401 no autorizado ya que no se envían las credenciales
	
	//curl --user pepe:contrasenia2024 -v http://localhost:8080/03a_JakartaEE-Authorization/mensajes/Empleado/enviarMensaje?mensaje=HolaMundo
	...
	public String enviarMensajeComoEmpleado(@QueryParam("mensaje") String mensaje) {
		return servicio.enviarMensajeComoEmpleado(mensaje);
	}
	
	
	//curl --cacert certificadoPrueba.pem --user pepe:contrasenia2024 -v https://localhost:8443/03a_JakartaEE-Authorization/mensajes/Empleado/enviarMensajeSeguro?mensaje=HolaMundo
